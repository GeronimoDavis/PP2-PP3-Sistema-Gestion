...
export default function InventarioPage() {  
  ...
  const handleUpdateProductStatus = async (productId: string) => {
    try {
      ...
      const productsWithCategories = response.products.map(product => ({
        ...product,
  ...
  const handleGetProductByCategory = async (categoryId: string) => {
    ...
    try {
      ...
      setError("Error al filtrar por categoría: " + (error.response?.data?.error || error.message));
    } finally {
      setLoading(false);
    }
  };
...
const handleClearFilters = async () => {
  ...
  try {
    ...
    setError("Error al limpiar filtros: " + (error.response?.data?.error || error.message));
  } finally {
    setLoading(false);
  }
};
  ...
  const handleCreateProduct = async () => {
    try {
      ...
      // Validaciones 
      if (!productData.name || !productData.code || !productData.stock || !productData.purchase_price || !productData.category_id) {
      ...
      const isNameValid = await validateNameInCategory(productData.name, productData.category_id);
  ...
  const handleUpdateProduct = async () => {
    try {
      ...
      // Validar codigo unico 
      const isCodeValid = await validateCode(productData.code, editingProduct.product_id);
      if (!isCodeValid) return;
  
      // Validar nombre unico en categoria 
      const isNameValid = await validateNameInCategory(productData.name, productData.category_id, editingProduct.product_id);
      if (!isNameValid) return;
  ...
  const validateCode = async (code: string, excludeId?: string) => {
    try {
      ...
      const existingProduct = products.find(p => 
        p.code === code && (!excludeId || p.product_id !== excludeId)
      );
  ...
  const validateNameInCategory = async (name: string, categoryId: string, excludeId?: string) => {
    try {
      ...
      if (existingProduct) {
        setNameError("Ya existe un producto con este nombre en la misma categoría");
        return false;
      } else {
        setNameError("");
        return true;
  ...
  const handleNameChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newName.length > 0 && productData.category_id) {
      await validateNameInCategory(newName, productData.category_id, editingProduct?.product_id);
    } else {
      setNameError("");
    }
  };
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input 
                    ...
                    onChange={(e) => setProductData({ ...productData, purchase_price: e.target.value })}
                  />
                </div>
              </div>
              <DialogFooter>
...
/>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              {categories.map((category) => (
                <DropdownMenuCheckboxItem key={category.category_id} checked={category.active} onCheckedChange={() => handleGetProductByCategory(category.category_id)}>
                  {category.name}
                </DropdownMenuCheckboxItem>
              ))}
            </DropdownMenuContent>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          >
                            ...
                            {product.active ? "Eliminar Producto" : "Activar Producto"}
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
